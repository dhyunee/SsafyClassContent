...
public class boj_2023_4 {
	...
	static void search(int num) {
		...
		while (!stack.isEmpty()) {
			...
			item it = stack.pop();
			if (it.idx == n) {
				sb.append(it.n).append("\n");
				continue;
			}

			int next_num = nextnum(it.n, 1);
			if (isPrime(next_num))
				stack.add(new item(next_num, it.idx + 1));

			next_num = nextnum(it.n, 3);
			if (isPrime(next_num))
				stack.add(new item(next_num, it.idx + 1));

			next_num = nextnum(it.n, 7);
			if (isPrime(next_num))
				stack.add(new item(next_num, it.idx + 1));

			next_num = nextnum(it.n, 9);
			if (isPrime(next_num))
				stack.add(new item(next_num, it.idx + 1));
