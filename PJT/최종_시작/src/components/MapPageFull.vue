<template>
  <div class="mapArea">
    <info-display
      v-show="isInfoShow"
      :sendStation="infoStation"
      :sendKeyword="infoKeyword"
      :sendInflu="infoInflu"
      v-on:closeFlag="closeInfoShow"
    ></info-display>
...
export default {
  ...
  methods: {
    ...
    apiGetStation(positions) {
      axios
        .get(
          `https://dapi.kakao.com/v2/local/search/category.json?category_group_code=SW8&x=${positions.getLng()}&y=${positions.getLat()}&sort=distance&size=3`,
          {
            headers: {
    ...
    apiGetKeyword(keyword, positions) {
      axios
        .get(
          `https://dapi.kakao.com/v2/local/search/keyword.json?query=${keyword}&x=${positions.getLng()}&y=${positions.getLat()}&sort=distance&size=3`,
          {
            headers: {
              Authorization: `KakaoAK ${this.appKey}`, //the token is a variable which holds the token
            },
    ...
    apiGetInfluence(positions) {
      ...
      axios
        .get(
          `https://dapi.kakao.com/v2/local/search/category.json?category_group_code=CS2&x=${positions.getLng()}&y=${positions.getLat()}&radius=300&sort=distance`,
          {
            headers: {
              Authorization: `KakaoAK ${this.appKey}`, //the token is a variable which holds the token
            },
